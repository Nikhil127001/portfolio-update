...
const ProjectPageItemsComponentWeb = () => {
  ...
  const getprojects = async () => {
    try {
      ...
      if (response.data.projects) {
        const projectsData = response.data.projects.filter((project) => project.projectType === 'Web')
        ...
        <div key={idx}>
          <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-evenly', alignItems: 'center' }} >
            ...
            <div className = 'ProjectDesc'>
              ...
              <div ><b>Github Link :</b> <a style={{display: 'flex', flexWrap: 'wrap'}}  href={project.githubLink} target='_blank' rel="noreferrer">{project.githubLink}</a></div>
